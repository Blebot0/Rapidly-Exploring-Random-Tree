...
class RRT_Planner():
    ...
    def rtt_planner(self):
        ...
        if flag == 0:
            ...
            nearest_node = Node(Node = self.node_list[nearest_node_index])
    ...
    def generate_goal_path(self):
        path = [self.end_point]
        node = Node(Node = self.node_list[-1])
        node.parent_node = self.node_list[-1].parent_node
        node.dist = self.node_list[-1].dist
        node.theta = self.node_list[-1].theta
    ...
    def chain(self, nearest_node, random_node):
        random_node.parent_node = Node(Node = nearest_node)
